...
class Program {
  ...
  private def getPath(args: Array[String]): Either[FilePathError, String] = {
    args.headOption
      .toRight(MissingPathArg)
  }

  private def readFile(path: String): Either[ReadFileError, java.io.File] = {
  ...
  private def index(directory: java.io.File): Index = {
  ...
  @tailrec
  private def iterate(indexedFiles: Index): Unit = {
    print(s"search> ")
    ...
    val result: String = indexedFiles.files
      .map { case IndexFile(name, words) =>
        ...
        val calc: Int = count * 100 / wordsToSearch.length
        (name, calc)
      }
